...
<body>
    <div class="left-menu-container">
        ...
        <div class="left-menu">
            ...
            <a class="menu-item" id="active-item" href="${pageContext.request.contextPath}/DanhSachDeThi">
                <div class="icon-container">
    ...
    <div class="top-nav">
        <h3 class="title">${exam.name}</h3>
    </div>
    <div class="main-container">
        <div class="func-container">
            <form action="${pageContext.request.contextPath}/DanhSachDeThi/DeThi" method="get">
            ...
            <a href="${pageContext.request.contextPath}/DanhSachDeThi/DeThi?examId=${exam.id}" class="refresh-btn func-btn">Làm mới</a>
        </div>
        <hr>
        <div class="main-content">
        	<c:if test="${classesNoneExam != null}">
	            <h4 class="sub-title">Danh sách lớp chưa có đề thi</h4>
	            <table id="classesNoneExam" class="table table-striped table-bordered" style="width:100%">
	                <thead>
	                    <tr>
	                        <th>Mã lớp</th>
	                        <th>Tên lớp</th>
	                ...
	                <tbody>
	                    <c:forEach var="classEntry" items="${classesNoneExam}">
		                    <tr>
		                        <td>${classEntry.id}</td>
		                        <td>${classEntry.name}</td>
		                        <td>
		                           <a href="${pageContext.request.contextPath}/GanDeThiChoLop?classId=${classEntry.id}&examId=${exam.id}" class="choose-btn sub-func-btn">Gán</a>
		                        </td>
		                    </tr>
	                    </c:forEach>
	                </tbody>
            ...
            <h4 class="sub-title">Danh sách lớp được gán đề thi</h4>
            <table id="classesHaveExam" class="table table-striped table-bordered" style="width:100%">
                <thead>
                    <tr>
                        <th>Mã lớp</th>
                        <th>Tên lớp</th>
                ...
                <tbody>
                	<c:forEach var="classEntry" items="${classesHaveExam}">
	                    <tr>
	                        <td>${classEntry.id}</td>
	                        <td>${classEntry.name}</td>
	                        <td>
	                            <button type="button" class="del-btn sub-func-btn" onclick="delClass('${classEntry.id}', '${exam.id}')">Xóa</button>
	                        </td>
	                    </tr>
                    </c:forEach>
                </tbody>
    ...
    <script>
        function delClass(classId, examId) {
            if (window.confirm("Bạn chắc chắn muốn xóa?")) {
                window.location = "${pageContext.request.contextPath}/BoGanDeThiChoLop?classId=" + classId + "&examId=" + examId;
            }
        }
    </script>
    
    <c:if test="${classExamError == true}">
	    <script>
    ...
    <script>
        $(document).ready(function () {
            $('#classesNoneExam').DataTable();
        });

         $(document).ready(function () {
            $('#classesHaveExam').DataTable();
        });
    </script>
</body>
