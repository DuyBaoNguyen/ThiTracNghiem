...
<body>
    ...
    <div class="main-container">
        <div class="func-container">
             <form action="${pageContext.request.contextPath}/DanhSachDeThi" method="get">
            ...
            <a href="${pageContext.request.contextPath}/ThemDeThi" class="add-btn func-btn">Thêm</a>
            <a href="${pageContext.request.contextPath}/DanhSachDeThi" class="refresh-btn func-btn">Làm mới</a>
        </div>
        <hr>
            ...
            <table id="exams" class="table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                	<c:forEach var="exam" items="${exams}">
	                    <tr>	                        
	                        <td><a href="${pageContext.request.contextPath}/DanhSachDeThi/DeThi?examId=${exam.id}">${exam.name}</a></td>
	                        <td><a href="${pageContext.request.contextPath}/DanhSachDeThi/DeThi?examId=${exam.id}">${exam.totalTime}</a></td>
	                        <td><a href="${pageContext.request.contextPath}/DanhSachDeThi/DeThi?examId=${exam.id}">${exam.totalQuestion}</a></td>
	                        <td><a href="${pageContext.request.contextPath}/DanhSachDeThi/DeThi?examId=${exam.id}">${exam.field.name}</a></td>
	                        <td>
	                            <button type="button" class="del-btn sub-func-btn" onclick="delExam('${exam.id}')">Xóa</button>
	                            <a href="${pageContext.request.contextPath}/SuaDeThi?examId=${exam.id}" class="update-btn sub-func-btn">Sửa</a>
	                        </td>
	                    </tr>
	               	</c:forEach>
                </tbody>
    ...
    <script>
        function delExam(examId) {
            if (window.confirm("Bạn chắc chắn muốn xóa?")) {
                window.location = "${pageContext.request.contextPath}/XoaDeThi?examId=" + examId;
            }
        }
    </script>

	<c:if test="${examError == true}">
	    <script>
