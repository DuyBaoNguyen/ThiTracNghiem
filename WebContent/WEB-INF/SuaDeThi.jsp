...
<body>
	...
	<div class="top-nav">
		<h3 class="title">Sửa đề thi</h3>
	</div>
	<div class="main-container">
		<div class="main-content">
			<div class="main-content">
				<form action="${pageContext.request.contextPath}/SuaDeThi"
					method="post" accept-charset="UTF-8">
					<input type="hidden" name="examId" value="${exam.id}">
					<h5 class="sub-title">Tên đề thi</h5>
					<input type="text" name="examName" value="${exam.name}" required autocomplete="off">
					<br>
					<h5 class="sub-title">Thời gian thi</h5>
					<input type="text" name="totalTime" value="${exam.totalTime}" required autocomplete="off">
					<br>
					<c:forEach var="struct" items="${exam.examStructures}">
						<h5 class="sub-title">Số câu hỏi ${struct.typeQuestion.name}</h5>
						<input type="hidden" name="typeQuestion${struct.typeQuestion.id}" value="${struct.typeQuestion.id}">
						<input type="text" name="totalQuestion${struct.typeQuestion.id}" value="${struct.numberQuestions}" required autocomplete="off">
						<br>
					</c:forEach>
					...
					<select name="fieldId">
	                 	<c:forEach var="field" items="${fields}">
		                    <option value="${field.id}"
		                    	<c:if test="${field.id == exam.field.id}">
		                    		selected
		                    	</c:if>
		                    >${field.name}</option>
	                    </c:forEach>
